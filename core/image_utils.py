"""
core/image_utils.py
OpenCV görüntüsü üzerinde kutu ve nokta çizim yardımcıları.
"""
...
def draw_boxes(img: np.ndarray, boxes: list) -> None:
    """Seçili ROI kutularını img üzerine çizer (yerinde)."""
...
def draw_analysis_result(img: np.ndarray, results: list) -> None:
    """
    Analiz sonuçlarını (etiket + merkez noktaları) img üzerine çizer.

    results: [(count, x1, y1, x2, y2, centers), ...]
    """
    ...
    for i, (count, x1, y1, x2, y2, centers) in enumerate(results):
        ...
        # Sadece geçerli merkezleri göster (count kadar)
        gosterilecek = centers[:count]
        for cx, cy in gosterilecek:
            cv2.circle(img, (x1 + cx, y1 + cy), 3, (0, 255, 255), -1)


def draw_preview_box(img: np.ndarray, x1: int, y1: int, x2: int, y2: int) -> None:
    """Sürükleme sırasında yarı şeffaf önizleme kutusu çizer."""
    overlay = img.copy()
